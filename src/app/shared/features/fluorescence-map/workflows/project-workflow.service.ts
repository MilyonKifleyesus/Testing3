...
import { FactoryLocation, NodeStatus, OperationalStatus, ProjectRoute, SubsidiaryCompany } from '../../../../shared/models/fluorescence-map.interface';
...
export class ProjectWorkflowService {
  ...
  private getCoordinateFromPayloadOrFallback(
    ...
    const hasPayloadLatitude = payload.latitude != null;
    const hasPayloadLongitude = payload.longitude != null;
    if (hasPayloadLatitude !== hasPayloadLongitude) {
      throw new Error('Both latitude and longitude are required when editing coordinates.');
    }

    const latitudeCandidate = hasPayloadLatitude ? payload.latitude : fallbackLatitude;
    const longitudeCandidate = hasPayloadLongitude ? payload.longitude : fallbackLongitude;
  ...
  async onProjectAdded(
    ...
    if (locationId == null || !Number.isFinite(locationId)) {
      ctx.handleModalError('A valid API location is required.');
    ...
    try {
      ...
      if (projectTypeId == null) {
        ...
        ctx.handleModalError(`${msg} Select a valid API type.`);
      ...
      ctx.handleModalError('Failed to add project. Please try again.');
      this.toastr.error(
