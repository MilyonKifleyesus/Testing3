...
export function getFirstFactoryOption(): FactoryOption {
  ...
  const locationNameLower = String(f.factory_location_name ?? '').toLowerCase();
  const parts = [f.city, f.country]
    .filter(Boolean)
    .filter((part) => !locationNameLower.includes(String(part).toLowerCase()));
  const locSuffix = parts.length > 0 ? ` (${parts.join(', ')})` : '';
  const label = `${mfrName} - ${f.factory_location_name}${locSuffix}`;
  return {
    manufacturerLocationId: Number(f.factory_id) + 100000,
