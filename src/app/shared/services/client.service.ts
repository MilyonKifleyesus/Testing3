...
/**
 * Strict API payload input. Legacy location fields are accepted for compatibility
 * but are not written back to the client API payload.
 */
...
export class ClientService {
  ...
  private readonly latestClientNameMap = new Map<string, string>();
  ...
  getClients(): Observable<Client[]> {
    ...
    const request$ = this.clientsListRequest$().pipe(
      ...
      map((clients) => {
        this.latestClientNameMap.clear();
        for (const client of clients) {
          const id = String(client.id ?? '').trim();
          const name = String(client.name ?? '').trim();
          if (!id || !name) continue;
          this.latestClientNameMap.set(id, name);
        }
        return clients;
      }),
      catchError((err) => {
  ...
  getClientNameMap(): Observable<Map<string, string>> {
    return this.getClients().pipe(
      map((clients) => {
        ...
        this.latestClientNameMap.clear();
        nameMap.forEach((name, id) => this.latestClientNameMap.set(id, name));
        return nameMap;
      })
    );
  }

  resolveClientName(clientId: string | number | null | undefined, fallback = ''): string {
    const normalizedId = String(clientId ?? '').trim();
    if (!normalizedId) {
      return fallback;
    }

    return this.latestClientNameMap.get(normalizedId) ?? (fallback || normalizedId);
  }
