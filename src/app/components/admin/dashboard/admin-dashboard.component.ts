...
export class AdminDashboardComponent implements OnInit, OnDestroy {
  ...
  private readonly STORAGE_KEY = 'buspulse_dashboard_layout';
  ...
  private loadAllVehiclesFromApi(): void {
    this.dashboardProjectsService.getAllVehicleOptionsResult({ clientId: 0 }).subscribe({
  ...
  private loadVehicleTotalCount(): void {
    this.dashboardProjectsService
      .getAllVehicleOptionsResult({ clientId: 0, includeAllOption: false })
      .subscribe({
        next: (result: DashboardVehicleOptionsResult) => {
  ...
  resetDashboard(): void {
    this.showConfirmModal(
      'Reset Dashboard',
      'Reset dashboard to default layout? This will remove all customizations.',
      () => {
  ...
  restoreAllWidgets(): void {
    this.showConfirmModal(
      'Restore Widgets',
      'Restore all widgets to default configuration?',
      () => {
        localStorage.removeItem(this.STORAGE_KEY);
        this.initializeWidgets();
      }
