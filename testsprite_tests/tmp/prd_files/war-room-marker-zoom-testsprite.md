...
This document defines test scenarios for **TestSprite** to verify that map markers and their logos **stay at the same geographic location** when the user zooms in and out. Previously, markers/logos could drift relative to the map; the fix (overlay inside map container, viewBox, stable coordinates) should keep them fixed.
...
- **Type**: Frontend / E2E
- **Entry point**: Dashboard with War Room (e.g. `/dashboard` or `/admin/dashboard` with War Room visible)
...
### TC-MZ-001: Marker and logo stay fixed when zooming in
...
### TC-MZ-002: Marker and logo stay fixed when zooming out
...
### TC-MZ-003: Marker and logo stay fixed when zooming in then out
...
### TC-MZ-004: Multiple markers all stay fixed on zoom
...
- **Zoom**: Trigger zoom via the on-map zoom controls (e.g. “+” / “−” buttons) or any exposed zoom API; avoid relying on mouse wheel if the test environment does not support it reliably.
- **Stability**: Position and scale are formatted (e.g. `toFixed(6)` for position, `toFixed(4)` for scale) so that coordinate systems and transforms remain stable across zoom levels.

---

## Reference

- **Related docs**: `docs/war-room-testing-instructions.md` (general War Room tests)
- **Implementation**: `src/app/shared/features/war-room/components/war-room-map/` (map component, markers component, overlay inside `#war-room-map`, viewBox from container size)
